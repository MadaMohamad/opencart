...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		public function getProductsByDeviceId($device_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_device p2c ON (p.product_id = p2c.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.device_id = '" . (int)$device_id . "' ORDER BY pd.name ASC");
	...
	public function getProductDevices($product_id) {
		$product_device_data = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_device WHERE product_id = '" . (int)$product_id . "'");
		
		foreach ($query->rows as $result) {
			$product_device_data[] = $result['device_id'];
		}

		return $product_device_data;
	}
	...
	public function getDevice($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_device WHERE product_id = '" . (int)$product_id . "'");
		
		return $query->rows;
	}	
	
	public function getDevices($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_device WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getDevicesByProductId($product_id) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_device WHERE product_id = '" . (int)$product_id . "'");
	...
	}public function getAllTabsOfProducts($product_id) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_tab WHERE product_id = '" . (int)$product_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY sort_order ASC, name ASC");
				return $query->rows;			
			}
}
?>
